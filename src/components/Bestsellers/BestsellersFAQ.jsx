...
const BestsellersFAQ = () => {
  ...
  return (
    <section className="max-w-3xl mx-auto p-6 sm:p-10 bg-gary-900 rounded-xl my-[-10px] ">
      <h2 className="text-3xl font-extrabold mb-8 text-yellow-500 text-center tracking-wide select-none">
      ...
      <div className="space-y-5">
        {faqData.map((item, idx) => (
          <div
            key={idx}
            className="border border-yellow-400 rounded-lg overflow-hidden bg-gray-900 shadow-sm"
          >
            <button
              onClick={() => toggle(idx)}
              ...
              id={`faq-question-${idx}`}
              className={`w-full flex justify-between items-center px-6 py-4 text-left font-semibold text-yellow-500 hover:bg-gray-950/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-yellow-400 transition-colors duration-200`}
            >
              <span className="text-lg md:text-xl">{item.question}</span>
              <svg
                className={`w-6 h-6 transform transition-transform duration-300 stroke-yellow-500 ${
            ...
            <div
              ...
              className="px-6 overflow-hidden text-yellow-500 text-base md:text-lg leading-relaxed select-text"
