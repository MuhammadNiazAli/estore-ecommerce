    ...
    >
      ...
      <ul className="space-y-6">
        {PRIVACY_OPTIONS.map(({ id, icon, label, description }) => (
          <li
            key={id}
            className="flex flex-col bg-gray-900 border border-gray-700 rounded-xl p-4"
          >
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                ...
                className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 ${
                  settings[id] ? 'bg-yellow-500' : 'bg-gray-600'
                }`}
              >
                <span
            ...
            <p className="text-xs text-yellow-300">{description}</p>
          </li>
        ))}
      </ul>
    </section>
