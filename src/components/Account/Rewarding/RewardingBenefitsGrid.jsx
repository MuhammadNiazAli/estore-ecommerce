...
function BenefitCard({ icon, title, description }) {
  return (
    <article
      className="bg-white border border-gray-200 rounded-xl p-5 shadow-lg transition-all duration-300 hover:border-amber-600"
      tabIndex={0}
      role="region"
      aria-label={title}
    >
      <div className="mb-4 flex justify-center items-center">{icon}</div>
      <h3 className="text-center text-amber-500 text-lg font-semibold mb-1">{title}</h3>
