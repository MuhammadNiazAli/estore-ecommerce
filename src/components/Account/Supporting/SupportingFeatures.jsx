...
const features = [
  {
    icon: <FiCheckCircle className="text-amber-200 text-4xl" />,
    title: 'Reliable Support',
    description: 'Get consistent and dependable assistance whenever you need it.',
  },
  {
    icon: <FiShield className="text-amber-200 text-4xl" />,
    title: 'Secure & Private',
    description: 'Your data and communications are always safe with us.',
  },
  {
    icon: <FiClock className="text-amber-200 text-4xl" />,
...
const SupportingFeatures = () => {
  return (
    <section
      ...
      className="max-w-[900px] mx-auto px-6 py-14 bg-white rounded-3xl shadow-2xl text-amber-300"
    >
        ...
        {features.map(({ icon, title, description }, idx) => (
          <div
            ...
            className="flex flex-col items-center text-center space-y-5 px-6 py-8 rounded-2xl
              bg-amber-900 bg-opacity-60 shadow-md
              hover:bg-opacity-80 hover:scale-[1.03] hover:shadow-xl transition-transform duration-300"
          >
            <div
              className="p-5 bg-gradient-to-tr from-amber-700 to-amber-600 rounded-full shadow-lg
                flex items-center justify-center transition-colors duration-300"
            >
              {icon}
            </div>
            <h3 className="text-xl font-semibold">{title}</h3>
            <p className="text-amber-500 text-sm leading-relaxed max-w-xs">{description}</p>
          </div>
        ))}
      </div>
    </section>
