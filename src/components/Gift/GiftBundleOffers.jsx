    ...
    >
      ...
      <div className="text-center mb-10 select-none">
        <div className="flex justify-center mb-4">
          <GiftIcon className="w-9 h-9 text-black animate-bounce" aria-hidden="true" />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
        {bundleData.slice(0, visibleCount).map((bundle) => (
          <article
            key={bundle.id}
            className="bg-white border border-black/20 rounded-2xl shadow-md p-6 cursor-pointer focus:outline-none focus-visible:ring-4 focus-visible:ring-black transition hover:shadow-black/30 hover:border-black/70"
            tabIndex={0}
          >
            ...
            <div className="flex items-center justify-between mt-auto">
              <span className="text-black font-bold text-lg select-none">{bundle.price}</span>
              <button
                type="button"
                className="inline-flex items-center gap-1 text-black hover:text-black font-semibold focus:outline-none focus:ring-2 focus:ring-black transition"
      ...
      {visibleCount < bundleData.length && (
        <div className="mt-10 flex justify-center">
          <button
            ...
            className="bg-black text-gray-900 font-extrabold px-8 py-3 rounded-full shadow-lg hover:bg-black focus:outline-none focus:ring-4 focus:ring-black transition"
