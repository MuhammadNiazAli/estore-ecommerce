"use client";

import React, { useRef, useEffect, useState } from "react";
import { Canvas, useFrame } from "@react-three/fiber";
import {
  OrbitControls,
  Environment,
  ContactShadows,
  useGLTF,
} from "@react-three/drei";

// ✅ Books Model Component
const BooksModel = ({ scale }) => {
  const groupRef = useRef();
  const { scene } = useGLTF("/models/book_encyclopedia_set_01_2k.gltf");

  useFrame(() => {
    if (groupRef.current) {
      groupRef.current.rotation.y += 0.003; // smooth rotation
    }
  });

  return (
    <primitive
      ref={groupRef}
      object={scene}
      scale={scale}
      position={[0, -1.3, 0]} // slight downward adjustment for fit
    />
  );
};

// ✅ BooksHero Section
const BooksHero = () => {
  const [modelScale, setModelScale] = useState([3.5, 3.5, 3.5]);
  const [cameraPosition, setCameraPosition] = useState([0, 0, 5]);
  const [canvasHeight, setCanvasHeight] = useState(500);

  // ✅ Responsive adjustments
  useEffect(() => {
    const handleResize = () => {
      const width = window.innerWidth;

      if (width < 640) {
        // Mobile
        setModelScale([4.8, 4.8, 4.8]);
        setCameraPosition([0, 0, 4]);
        setCanvasHeight(400);
      } else if (width < 1024) {
        // Tablet
        setModelScale([4.2, 4.2, 4.2]);
        setCameraPosition([0, 0, 4.5]);
        setCanvasHeight(460);
      } else {
        // Desktop
        setModelScale([3.5, 3.5, 3.5]);
        setCameraPosition([0, 0, 5]);
        setCanvasHeight(500);
      }
    };

    handleResize();
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, []);

  return (
    <section className="w-full bg-gray-900 text-white py-16 lg:py-24 my-[-50px]">
      <div className="mx-auto max-w-[1200px] px-4 flex flex-col lg:flex-row items-center gap-12">
        
        {/* ✅ Left Content */}
        <div className="flex-1 space-y-6 text-center lg:text-left">
          {/* Highlight Badge */}
          <div className="inline-block bg-yellow-400 text-black px-4 py-1 rounded-full text-sm font-semibold">
            Premium Encyclopedia Set 2025
          </div>

          {/* Heading */}
          <h1 className="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight">
            Expand Your{" "}
            <span className="text-yellow-400 underline decoration-yellow-300 decoration-4">
              Knowledge Horizon
            </span>
          </h1>

          {/* Description */}
          <p className="text-gray-300 max-w-md mx-auto lg:mx-0 text-lg leading-relaxed">
            Own the ultimate reference set with updated content, premium binding,
            and timeless value for your library or office.
          </p>

          {/* Price Block */}
          <div className="bg-gray-800/60 p-4 rounded-xl inline-block">
            <p className="text-3xl font-bold text-yellow-400">$499</p>
            <p className="text-gray-400 text-sm">Free worldwide shipping!</p>
          </div>

          {/* Buttons */}
          <div className="flex justify-center lg:justify-start gap-5 flex-wrap">
            <button className="bg-yellow-400 text-black px-7 py-3 rounded-full font-semibold shadow-md hover:bg-yellow-300 transition duration-300 ease-in-out">
              Buy Now
            </button>
            <button className="border border-white px-7 py-3 rounded-full font-semibold shadow-md hover:bg-white hover:text-black transition duration-300 ease-in-out">
              Learn More
            </button>
          </div>

          {/* Features */}
          <div className="flex justify-center lg:justify-start gap-6 mt-6 text-gray-400 text-sm flex-wrap">
            <p>✔ Premium Hardcover</p>
            <p>✔ Updated Content</p>
            <p>✔ Secure Payments</p>
          </div>
        </div>

        {/* ✅ Right 3D Model */}
        <div className="flex-1 w-full" style={{ height: `${canvasHeight}px` }}>
          <Canvas
            shadows
            camera={{ position: cameraPosition, fov: 30 }}
            gl={{ antialias: true }}
          >
            <ambientLight intensity={0.8} />
            <directionalLight
              castShadow
              position={[5, 8, 5]}
              intensity={2}
              shadow-mapSize-width={2048}
              shadow-mapSize-height={2048}
            />
            {/* HDRI for realistic lighting */}
            <Environment files="https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/venice_sunset_1k.hdr" />
            <OrbitControls enableZoom={false} enablePan={false} />
            <BooksModel scale={modelScale} />
            <ContactShadows
              position={[0, -1.5, 0]}
              opacity={0.4}
              scale={12}
              blur={3}
              far={8}
            />
          </Canvas>
        </div>
      </div>
    </section>
  );
};

export default BooksHero;
