...
const SportsFilters = () => {
  ...
  const showLess = () => {
    setVisibleCount(8);
    gsap.to(window, { scrollTo: { y: "#sports-section" }, duration: 0.5 });
  };
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredProducts.slice(0, visibleCount).map((product) => (
            <div
              key={product.id}
              className="product-card bg-white rounded-xl shadow-md p-4 relative group hover:shadow-lg transition text-black"
            >
              <img
                src={product.image}
                alt={product.name}
