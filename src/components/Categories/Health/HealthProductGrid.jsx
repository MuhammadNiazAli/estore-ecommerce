...
const HealthProductGrid = () => {
  const [sortOption, setSortOption] = useState("");
  const [visibleCount, setVisibleCount] = useState(6);
  const [products, setProducts] = useState(healthData.slice(0, 6));
  const [hoveredId, setHoveredId] = useState(null);

  const handleLoadMore = () => {
    const newCount = visibleCount + 6;
    setVisibleCount(newCount);
    setProducts(healthData.slice(0, newCount));
  };
  ...
  const handleSort = (value) => {
    ...
    if (value === "low-high") {
      ...
      sortedProducts = healthData.slice(0, visibleCount);
    }
    ...
    <section className="bg-gray-900 text-white py-12 px-4">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <h2 className="text-2xl font-bold">Health Essentials Collection</h2>
        ...
        <div className="text-center mt-8">
          {visibleCount < healthData.length && (
            <button
...
export default HealthProductGrid;
