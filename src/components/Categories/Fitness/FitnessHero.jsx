...
// ✅ Fitness Model Component
const FitnessModel = ({ scale }) => {
  const groupRef = useRef();
  const { scene } = useGLTF("/models/picke_dirty_01_2k.gltf");
  ...
  return <primitive ref={groupRef} object={scene} scale={scale} position={[0, -1.2, 0]} />;
};
...
const FitnessHero = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) {
        // Mobile → Bigger model for better visibility
        setModelScale([5, 5, 5]);
      ...
      <div className="mx-auto max-w-[1200px] px-4 flex flex-col lg:flex-row items-center gap-12">
        ...
        <div className="flex-1 space-y-6 text-center lg:text-left">
          ...
          <p className="text-gray-300 max-w-md mx-auto lg:mx-0 text-lg leading-relaxed">
            Explore high-performance fitness equipment engineered for durability,
            comfort, and peak performance at home or in the gym.
          </p>

          <div className="bg-gray-800/60 p-4 rounded-xl inline-block">
            <p className="text-3xl font-bold text-yellow-400">$899</p>
            <p className="text-gray-400 text-sm">Free worldwide shipping!</p>
          </div>
        ...
        <div className="flex-1 w-full" style={{ height: `${canvasHeight}px` }}>
          ...
          >
            ...
            <FitnessModel scale={modelScale} />
            <ContactShadows position={[0, -1.5, 0]} opacity={0.4} scale={12} blur={3} far={8} />
          </Canvas>
        </div>
      </div>
    </section>
