...
const PhoneFilters = ({ filters, onFilterChange }) => {
  ...
  return (
    <section className="bg-gray-900 my-10 p-6 rounded-xl shadow-lg max-w-[1100px] mx-auto text-white">
  ...
  type="button"
  onClick={handleReset}
  className="flex items-center gap-1 text-xs bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded-full transition"
>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <input
            ...
            className="mb-2 w-full px-3 py-2 rounded bg-gray-800 border border-gray-600 text-white focus:ring-2 focus:ring-yellow-400"
          />
          <select
            id="brand"
            value={localFilters.brand}
            onChange={(e) => handleChange('brand', e.target.value)}
            className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded focus:ring-2 focus:ring-yellow-400"
          >
        ...
        <div>
          ...
          <input
            ...
            className="mb-2 w-full px-3 py-2 rounded bg-gray-800 border border-gray-600 focus:ring-2 focus:ring-yellow-400"
          />
          <select
            id="os"
        ...
        <div className="md:col-span-3 mt-4">
          <label className="block mb-2 font-semibold">
            Price Range: <span className="text-yellow-400">${localFilters.priceRange[0]} - ${localFilters.priceRange[1]}</span>
          </label>
          <input
            type="range"
            min="0"
        ...
        <div className="md:col-span-3 flex justify-center gap-4 mt-6 flex-wrap">
          <button
            type="submit"
            className="bg-yellow-400 text-black px-8 py-2 rounded-full font-semibold shadow hover:bg-yellow-300 transition text-sm"
          >
            Apply Filters
          </button>
          <button
            type="button"
            onClick={handleReset}
            className="bg-gray-700 text-white px-8 py-2 rounded-full font-semibold shadow hover:bg-gray-600 transition text-sm"
          >
            Reset Filters
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {Object.entries(localFilters)
            ...
            .map(([key, val]) => (
              <span
                key={key}
                className="bg-gray-800 text-yellow-400 px-3 py-1 rounded-full text-xs flex items-center gap-2"
              >
                {key}: {val}
                <button
