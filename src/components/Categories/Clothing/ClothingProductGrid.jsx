...
import { FaCartPlus, FaHeart, FaEye } from "react-icons/fa"; // Import icons
...
const ClothingProductGrid = () => {
  const [sortOption, setSortOption] = useState("");
  const [visibleCount, setVisibleCount] = useState(6);
  const [products, setProducts] = useState(clothingData.slice(0, 6));
  const [hoveredId, setHoveredId] = useState(null);

  const handleLoadMore = () => {
    const newCount = visibleCount + 6;
    setVisibleCount(newCount);
    setProducts(clothingData.slice(0, newCount));
  };
  ...
  const handleSort = (value) => {
    ...
    if (value === "low-high") {
      ...
      // If default, reset to slice of clothingData based on visibleCount
      sortedProducts = clothingData.slice(0, visibleCount);
    }
    ...
    <section className="bg-white text-gray-900 py-12 px-4">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <h2 className="text-2xl font-bold">Trending Clothing Collection</h2>
          ...
          {products.map((item) => (
            ...
            >
              ...
              <div className="p-4 space-y-2">
                ...
                {/* Features (optional, handle if missing) */}
        ...
        <div className="text-center mt-8">
          {visibleCount < clothingData.length && (
            <button
...
export default ClothingProductGrid;
