...
const LaptopFeaturedDeals = () => {
  ...
  return (
    <section className="relative max-w-[1000px] mx-auto px-4 py-10 my-[-40px] mb-0">
      <h2 className="text-3xl font-semibold mb-8 text-gray-100">Featured Deals</h2>

      {/* Scroll Arrows */}
      <button
        onClick={() => scroll('left')}
        className="absolute z-10 top-1/2 -left-4 transform -translate-y-1/2 bg-gray-800/80 hover:bg-gray-700 text-white p-2 rounded-full shadow-md transition"
      ...
      <button
        ...
        className="absolute z-10 top-1/2 -right-4 transform -translate-y-1/2 bg-gray-800/80 hover:bg-gray-700 text-white p-2 rounded-full shadow-md transition"
      ...
      >
        {deals.map(({ id, name, price, oldPrice, image }) => (
          <div
            key={id}
            className="relative min-w-[280px] bg-gray-800 rounded-lg overflow-hidden shadow-lg group cursor-pointer flex-shrink-0"
          >
            <img
              src={image}
              alt={name}
            ...
            {/* Glass-blur Overlay */}
            <div className="absolute inset-0 backdrop-blur-md bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-4">
              <button
              ...
              <Link
                href={`/laptop/${id}`}
                className="p-2 rounded-full bg-gray-700 hover:bg-blue-600 transition text-white"
              >
              ...
              <Link
                href={`/compare/${id}`}
                className="p-2 rounded-full bg-gray-700 hover:bg-green-600 transition text-white"
              >
            ...
            <div className="p-4">
              <h3 className="text-lg font-semibold text-gray-100 mb-2 truncate" title={name}>
                {name}
              </h3>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-gray-400 ml-2 text-sm">(120)</span>
              </div>
            </div>
          </div>
        ))}
