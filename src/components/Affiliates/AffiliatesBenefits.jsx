...
const AffiliatesBenefits = () => {
  ...
  return (
    <section className="bg-white text-black py-14 px-4 sm:px-6 lg:px-8">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-8" role="list">
          <AnimatePresence>
            {initialBenefits.slice(0, visibleCount).map(({ id, title, description, icon }) => (
              <motion.li
                key={id}
                className="flex items-start gap-4 p-5 rounded-lg bg-white hover:bg-gray-500 transition-colors duration-300 focus-within:ring-2 ring-black"
                tabIndex={0}
        ...
        {visibleCount < initialBenefits.length && (
          <div className="text-center mt-8">
            <button
              ...
              className="inline-flex items-center gap-2 bg-black text-gray-900 font-semibold px-6 py-2 rounded-full hover:bg-black transition-all focus:outline-none focus:ring-2 focus:ring-black disabled:opacity-50"
              disabled={loading}
            >
              {loading ? (
                <>
