...
const steps = [
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 sm:h-10 sm:w-10 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    return (
      <section className="bg-white text-amber-600 py-12 px-4">
        <div className="max-w-[1000px] mx-auto">
          <div className="h-10 sm:h-12 mb-10 bg-amber-900 rounded animate-pulse w-2/3 sm:w-1/3 mx-auto"></div>
        ...
        >
          {steps.map(({ id, title, description, icon }) => (
            <motion.article
              ...
              className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6 p-5 sm:p-6 rounded-lg border border-amber-900 bg-white hover:bg-amber-950 focus:bg-amber-950 transition-colors duration-300 cursor-default"
              variants={stepVariants}
            >
              {/* Step number circle */}
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-amber-700 text-gray-900 font-bold text-base sm:text-lg select-none">
                {id}
              </div>
              ...
              <div className="flex-1">
                ...
                <p id={`step-desc-${id}`} className="text-amber-500 text-sm sm:text-base leading-relaxed">
                  {description}
                </p>
              </div>
