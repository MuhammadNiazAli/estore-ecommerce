...
const ProductCallTestimonials = () => {
  return (
    <section className="w-full bg-gray-900 flex justify-center px-4 py-12 sm:py-16">
      <div className="max-w-[1000px] w-full">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-yellow-400 text-center mb-10 px-2 sm:px-0">
        ...
        <div className="flex flex-col sm:flex-row gap-6">
          {testimonials.map(({ name, photo, rating, comment }, idx) => (
            <article
              key={idx}
              className="bg-gray-800 rounded-xl p-6 flex flex-col items-center shadow-md hover:shadow-lg transition-shadow duration-300 flex-1 min-w-0"
              tabIndex={0}
            ...
            >
              <img
                src={photo}
                alt={`${name}'s photo`}
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover mb-4 border-2 border-yellow-400"
                loading="lazy"
              />
              <h3 className="text-yellow-400 font-semibold text-lg mb-1 text-center truncate max-w-full">
                {name}
              </h3>
