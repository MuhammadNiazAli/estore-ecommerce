...
const features = [
  ...
  {
    icon: <ThumbsUp className="w-8 h-8 text-yellow-400" />,
...
const ProductCallOverview = () => {
  return (
    <section className="w-full bg-gray-900 flex justify-center px-4 py-12 sm:px-6 my-[-40px] mb-0">
      <div className="max-w-[1000px] w-full text-center">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-yellow-400 mb-4">
          Why Choose Our Products?
        </h2>
        <p className="text-gray-300 max-w-3xl mx-auto mb-10 text-base sm:text-lg px-2 sm:px-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {features.map(({ icon, title, description }, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center bg-gray-800 rounded-xl p-6 hover:bg-gray-700 transition-colors duration-200"
            >
