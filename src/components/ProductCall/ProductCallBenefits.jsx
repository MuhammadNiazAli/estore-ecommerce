...
const benefits = [
  ...
  {
    icon: <Truck className="w-6 h-6 text-yellow-400" aria-hidden="true" />,
  ...
  {
    icon: <Sparkles className="w-6 h-6 text-yellow-400" aria-hidden="true" />,
...
const ProductCallBenefits = () => {
  return (
    <section className="w-full bg-gray-900 flex justify-center px-4 py-12 sm:px-6">
      <div className="max-w-[1000px] w-full grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
        {benefits.map((benefit) => (
          <div
            key={benefit.title}
            className="flex items-start gap-4 p-5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-all duration-200"
          >
