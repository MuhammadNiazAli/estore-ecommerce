...
const AccessoriesRelated = () => {
  return (
    <section className="max-w-[1000px] mx-auto px-4 py-10">
      <h3 className="text-4xl font-extrabold text-black mb-8 text-center md:text-left tracking-wide">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {relatedAccessories.map((item) => (
          <div
            key={item.id}
            className="relative group rounded-xl overflow-hidden shadow-lg bg-white cursor-pointer focus:outline-black"
            tabIndex={0}
          ...
          >
            ...
            <div className="absolute top-3 left-3 space-y-1 z-20">
              {item.bestSeller && (
                <span className="bg-black text-gray-900 font-bold px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wide shadow-lg select-none">
                  Best Seller
                </span>
              )}
              <span className="bg-black text-gray-900 font-semibold px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wide shadow-lg select-none">
                {item.discount}
              </span>
            </div>

            {/* Overlay Info */}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/95 to-transparent p-3 text-black transition-opacity duration-300 opacity-0 group-hover:opacity-100">
              <h4
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  className="bg-black hover:bg-black text-gray-900 font-semibold rounded-md px-2 py-0.5 text-xs transition select-none whitespace-nowrap"
