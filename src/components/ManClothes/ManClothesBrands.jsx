    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-gray-50 mb-3">
          Shop by <span className="text-yellow-500">Top Brands</span>
        </h2>
        ...
        {brands.map((brand) => (
          <motion.div
            ...
            className={`relative group bg-white/10 backdrop-blur-lg rounded-xl shadow-lg p-4 flex flex-col items-center justify-center cursor-pointer transition ${
              activeBrand === brand.name ? "ring-4 ring-yellow-400" : ""
            }`}
          >
            {/* Brand Logo */}
            <img
            ...
            {/* Brand Name */}
            <p className="mt-3 text-gray-900 font-semibold text-sm sm:text-base group-hover:text-yellow-500">
              {brand.name}
            </p>
          </motion.div>
        ))}
      ...
      {activeBrand && (
        ...
        >
          <p className="text-gray-700 text-sm">
            ...
            <span className="font-bold text-yellow-500">{activeBrand}</span>
          </p>
        </motion.div>
      )}
    </section>
