...
const contacts = [
  {
    ...
    icon: (
      <svg
        ...
        className="h-7 w-7 sm:h-8 sm:w-8 text-yellow-400"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
...
const PressContacts = () => {
  return (
    <section className="w-full bg-gray-900 flex justify-center px-4 sm:px-6 lg:px-8 py-16 mb-[-30px]">
      <div className="max-w-6xl w-full text-center">
        <h2 className="text-yellow-400 text-3xl sm:text-4xl md:text-5xl font-extrabold drop-shadow-lg">
          Press Contacts
        </h2>
        <p className="mt-4 text-yellow-300 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed drop-shadow-md">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {contacts.map(({ type, value, href, icon }) => (
            <a
              ...
              className="flex flex-col items-center justify-center bg-gray-800 hover:bg-gray-700 hover:ring-2 hover:ring-yellow-400 rounded-2xl p-6 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-yellow-400"
            >
              <div className="mb-3">{icon}</div>
              <h3 className="text-yellow-400 font-semibold text-lg sm:text-xl">{type}</h3>
              <p className="text-yellow-300 mt-1 text-sm sm:text-base break-words">{value}</p>
            </a>
          ))}
        </div>
      </div>
