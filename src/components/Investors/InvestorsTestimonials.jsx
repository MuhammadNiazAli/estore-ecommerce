...
const InvestorsTestimonials = () => {
  return (
    <section
      ...
      className="bg-white text-black py-12 px-4 flex justify-center mb-[-150px]"
    >
      <div className="max-w-[1000px] w-full">
        <h2 className="text-4xl font-extrabold mb-8 text-black text-center select-none">
        ...
        >
          {testimonials.map(({ id, name, title, photo, quote }) => (
            <article
              ...
              role="listitem"
              tabIndex={0}
              className="bg-white rounded-lg p-6 shadow-md hover:shadow-black/50 focus:shadow-black/70
            ...
            >
              <img
                ...
                height={80}
              />
              <p className="text-black italic mb-4 leading-relaxed font-medium select-text">
                “{quote}”
              </p>
              <h3 className="text-black font-semibold text-lg truncate w-full" title={name}>
                {name}
              </h3>
              <p className="text-black text-sm truncate w-full" title={title}>
                {title}
              </p>
            </article>
          ))}
