        ...
        <ul role="list" className="space-y-8">
          {newsArticles.map(({ id, date, displayDate, title, summary, link }) => (
            <motion.li
              ...
              className="bg-gray-900 rounded-2xl p-6 md:p-7 shadow-md hover:shadow-yellow-400/40 transition-all duration-300 group"
            >
              <time
                dateTime={date}
                className="block text-yellow-300 font-mono text-xs sm:text-sm mb-2 uppercase tracking-wide"
              >
                {displayDate}
              </time>
              <a
              ...
              >
                ...
                <ArrowUpRight className="w-5 h-5 text-yellow-400 mt-1 group-hover:translate-x-1 transition-transform duration-300" />
              </a>
              <p className="mt-3 text-yellow-300 text-sm sm:text-base leading-relaxed">
                {summary}
              </p>
            </motion.li>
          ))}
