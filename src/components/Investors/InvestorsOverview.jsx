...
const InvestorsOverview = () => {
  return (
    <section
      ...
      className="bg-white text-black py-16 px-4 flex justify-center my-15"
    >
      <div className="w-full max-w-[1100px] space-y-12">
        <motion.h2
          className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-center text-black"
        ...
        <motion.p
          className="text-black text-center max-w-3xl mx-auto text-base sm:text-lg lg:text-xl leading-relaxed"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          {metrics.map(({ label, value }, idx) => (
            <motion.div
              key={idx}
              role="listitem"
              className="bg-white rounded-xl p-6 sm:p-8 shadow-lg hover:shadow-black/50 focus-within:shadow-black/50 transition-shadow duration-300 text-center cursor-default"
              tabIndex={0}
            ...
            >
              <p className="text-black font-extrabold text-3xl sm:text-4xl lg:text-5xl mb-3">
                {value}
              </p>
