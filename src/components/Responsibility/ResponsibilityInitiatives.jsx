...
const initiatives = [
  ...
  },
  {
    ...
    icon: <Globe className="w-10 h-10 sm:w-12 sm:h-12 text-yellow-400" />,
  },
];

const ResponsibilityInitiatives = () => {
  return (
    <div className="w-full bg-gray-900 flex justify-center px-4 py-12 sm:py-16 md:py-20">
      <div className="max-w-[1000px] w-full text-center px-2 sm:px-0">
        <motion.h2
          ...
          className="text-yellow-400 text-3xl sm:text-4xl font-extrabold mb-10 sm:mb-12"
        >
          Our Key Initiatives
        </motion.h2>

        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-4 text-gray-300">
          {initiatives.map(({ id, title, description, icon }, index) => (
            <motion.div
              ...
              className="bg-gray-800 rounded-xl p-5 sm:p-6 cursor-default hover:shadow-lg hover:scale-[1.04] transition-transform duration-300"
            >
              <div className="mb-4 flex justify-center">{icon}</div>
              <h3 className="text-yellow-400 text-lg sm:text-xl font-semibold mb-2">{title}</h3>
