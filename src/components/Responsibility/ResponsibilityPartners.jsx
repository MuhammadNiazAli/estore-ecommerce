...
const ResponsibilityPartners = () => {
  ...
  return (
    <section
      ...
      className="w-full bg-gray-900 py-12 sm:py-14 px-4 flex flex-col items-center"
    >
      <motion.h2
        ...
        className="text-yellow-400 text-2xl sm:text-3xl md:text-4xl font-bold mb-8 sm:mb-10 select-none text-center max-w-[1000px] w-full"
      >
      ...
      <div className="relative w-full max-w-[1000px]">
        ...
        <button
          ...
          className={`hidden sm:flex absolute top-1/2 left-2 -translate-y-1/2 bg-gray-800/70 hover:bg-yellow-500 hover:text-gray-900 rounded-full p-2 z-20 transition
            focus:outline-yellow-400 focus-visible:outline-2 focus-visible:outline-offset-2
            ${!canScrollLeft ? 'opacity-40 cursor-not-allowed hover:bg-gray-800 hover:text-gray-400' : ''}`}
        >
        ...
        >
          {partners.map(({ id, name, logo, href }) => (
            <a
              ...
              tabIndex={0}
              title={name}
              className="flex-shrink-0 w-24 sm:w-28 md:w-32 h-16 sm:h-20 bg-gray-800 rounded-lg border border-gray-700 hover:border-yellow-400 focus:border-yellow-400 focus:outline-none transition flex items-center justify-center p-2 cursor-pointer"
            >
              <img
                src={logo}
                alt={name}
        ...
        <button
          ...
          className={`hidden sm:flex absolute top-1/2 right-2 -translate-y-1/2 bg-gray-800/70 hover:bg-yellow-500 hover:text-gray-900 rounded-full p-2 z-20 transition
            focus:outline-yellow-400 focus-visible:outline-2 focus-visible:outline-offset-2
            ${!canScrollRight ? 'opacity-40 cursor-not-allowed hover:bg-gray-800 hover:text-gray-400' : ''}`}
        >
