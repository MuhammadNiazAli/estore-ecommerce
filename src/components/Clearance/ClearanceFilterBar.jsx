...
export default function ClearanceFilterBar() {
  ...
  return (
    <div className="bg-gray-900 text-yellow-400 w-full flex flex-col items-center py-4 px-3 sm:px-6 my-5">
      {/* FILTER BAR */}
      <div className="max-w-[1200px] w-full flex flex-col gap-3 mb-6">
            ...
            <button
              ...
              className={`px-3 py-2 text-xs sm:text-sm rounded-md font-medium transition ${
                selectedCategory === cat
                  ...
                  : "border border-yellow-400 hover:bg-yellow-400 hover:text-gray-900"
              }`}
            >
              {cat}
            </button>
        ...
        <div className="flex justify-center sm:justify-between items-center gap-3 flex-wrap">
          <select
            ...
            className="bg-gray-800 border border-yellow-400 text-yellow-400 px-3 py-2 text-xs sm:text-sm rounded-md"
          >
            {sortOptions.map((opt) => (
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-md ${viewMode === "grid" ? "bg-yellow-400 text-gray-900" : "hover:bg-gray-800"}`}
            >
              <Grid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 rounded-md ${viewMode === "list" ? "bg-yellow-400 text-gray-900" : "hover:bg-gray-800"}`}
            >
              <List className="w-4 h-4" />
            </button>

            <button
              className="sm:hidden p-2 rounded-md border border-yellow-400 hover:bg-yellow-400 hover:text-gray-900"
      ...
      <AnimatePresence>
        {isFilterOpen && (
          <motion.div
            ...
            className="fixed top-0 right-0 w-4/5 sm:w-1/3 h-full bg-gray-800 text-yellow-400 p-5 shadow-lg z-50 overflow-y-auto"
          >
            <button
              onClick={() => setIsFilterOpen(false)}
              className="text-gray-200 mb-4 border border-yellow-400 px-3 py-1 rounded-md"
            >
              Close
            </button>
      ...
      >
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => (
            <div
              key={product.id}
              className="bg-gray-800 rounded-md overflow-hidden shadow hover:shadow-lg transition"
            >
              <img
                src={product.image}
                alt={product.name}
              ...
              <div className="p-3 text-xs sm:text-sm">
                <h3 className="text-yellow-400 font-semibold truncate">{product.name}</h3>
                <p className="text-yellow-300">${product.price.toFixed(2)}</p>
                <p className="text-yellow-400 text-[11px]">{product.rating} ★</p>
              </div>
            </div>
          ))
        ) : (
          <p className="text-yellow-300 text-center col-span-full text-sm">No products match your filters.</p>
        )}
      </div>
    </div>
  );
