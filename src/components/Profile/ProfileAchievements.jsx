...
const achievementsData = [
  ...
  },
  {
    ...
    icon: <CheckBadgeIcon className="w-6 h-6 text-yellow-400" />,
    progress: 50,
    category: 'In Progress',
  },
  {
  ...
  },
  {
    ...
    icon: <HeartIcon className="w-6 h-6 text-yellow-400" />,
    progress: 60,
    category: 'In Progress',
  },
];
...
const ProfileAchievements = () => {
  ...
  return (
    <section
      className="w-full bg-gray-900 text-white px-4 sm:px-8 md:px-12 py-8 sm:py-12 rounded-lg max-w-[1200px] mx-auto"
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-3 text-yellow-300 font-semibold text-xs sm:text-sm md:text-base select-none whitespace-nowrap">
          <TrophyIcon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 animate-bounce text-yellow-400 drop-shadow-md" />
      ...
      >
        {filteredAchievements.length === 0 && (
          <p className="text-yellow-400 col-span-full text-center py-10 font-semibold">
      ...
      >
        <button
          ...
          className="flex items-center gap-2 sm:gap-3 px-5 sm:px-7 py-2.5 sm:py-3 rounded-full text-base sm:text-lg bg-yellow-400 hover:bg-yellow-300 text-gray-900 font-extrabold shadow-xl transition-transform hover:scale-[1.05] focus-visible:ring-4 focus-visible:ring-yellow-500 focus:outline-none"
...
const AchievementCard = ({ title, description, icon, progress }) => {
  ...
  return (
    <div
      tabIndex={0}
      className="relative flex flex-col gap-3 bg-gray-800 p-4 sm:p-5 rounded-xl shadow-inner border border-yellow-400/20 hover:border-yellow-400 transition cursor-pointer focus:outline-none focus:ring-4 focus:ring-yellow-400"
    ...
    >
      <div className="flex items-center gap-3 sm:gap-4">
        <div className="p-2 sm:p-3 bg-yellow-400/20 rounded-full flex items-center justify-center min-w-[38px] min-h-[38px] sm:min-w-[44px] sm:min-h-[44px]">
          {icon}
        </div>
        <h3 className="text-white font-semibold text-base sm:text-lg md:text-xl truncate">{title}</h3>
        <InformationCircleIcon className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-300 opacity-70" />
      </div>
      ...
      {showTooltip && (
        <div
          role="tooltip"
          className="absolute top-full left-5 z-20 mt-2 max-w-xs bg-gray-700 text-yellow-300 text-sm sm:text-base p-3 rounded-md shadow-lg pointer-events-none select-none"
        >
          {description}
        </div>
      )}
      ...
      <p
        ...
        className="text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed min-h-[48px]"
        aria-live="polite"
      >
        {description}
      </p>
      ...
      {/* Progress Bar */}
      <div
        className="w-full bg-yellow-400/20 rounded-full h-2.5 sm:h-3 overflow-hidden"
      ...
      >
        <motion.div
          ...
          className="h-2.5 sm:h-3 bg-yellow-400 rounded-full shadow-lg"
        />
      </div>

      <div className="flex items-center gap-1 sm:gap-2 text-yellow-400 font-semibold text-xs sm:text-sm md:text-base">
